...
function reload_img () {
  if(!halted) mjpeg_img.src = "192.168.1.108:81/html/cam_pic.php?time=" + new Date().getTime() + "&pDelay=" + preview_delay;
  else setTimeout("reload_img()", 500);
}

function error_img () {
  setTimeout("mjpeg_img.src = '192.168.1.108:81/html/cam_pic.php?time=' + new Date().getTime();", 100);
}
...
function updatePreview(cycle)
{
   if (mjpegmode)
   {
      ...
      {
         mjpeg_img.src = "192.168.1.108:81/updating.jpg";
         setTimeout("mjpeg_img.src = \"192.168.1.108:81/html/cam_pic_new.php?time=\" + new Date().getTime()  + \"&pDelay=\" + preview_delay;", 1000);
         return;
      }
      ...
      if (previous_halted != halted)
      {
         if(!halted)
         {
            mjpeg_img.src = "192.168.1.108:81/html/cam_pic_new.php?time=" + new Date().getTime() + "&pDelay=" + preview_delay;			
         }
         else
         {
            mjpeg_img.src = "192.168.1.108:81/unavailable.jpg";
         }
      }
...
function reload_ajax (last) {
  ajax_status.open("GET","http://192.168.1.108:81/html/status_mjpeg.php?last=" + last,true);
  ajax_status.send();
}
...
function get_zip_progress(zipname) {
   ...
   ajax_zip.open("GET","http://192.168.1.108:81/html/preview.php?zipprogress=" + zipname);
   ajax_zip.send();
}
